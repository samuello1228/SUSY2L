...
ClassImp(ljetSelection)


const float iGeV = 0.001;
const float GeV = 1000;

ljetSelection :: ljetSelection (std::string name)
{
  ...
  m_isMC = 0;
  m_grlFile = "multiLepSearch/data15_13TeV.periodAllYear_DetStatus-v67-pro19-02_DQDefects-00-01-02_PHYS_StandardGRL_All_Good.xml";
  start = true;
}

EL::StatusCode ljetSelection :: setupJob (EL::Job& job)
{
...
EL::StatusCode ljetSelection :: histInitialize ()
{
  ...
  m_susyEvt->makeTree(outputTreeName);
...
EL::StatusCode ljetSelection :: fileExecute ()
{
...
EL::StatusCode ljetSelection :: changeInput (bool /*firstFile*/)
{
...
EL::StatusCode ljetSelection :: initialize ()
{
...
//   CHECK(m_objTool->setProperty("DataSource",m_isMC));
  CHECK(m_objTool->setProperty("EleId","LooseLH") );
  CHECK(m_objTool->setProperty("EleIdBaseline","LooseLH") );
...
//   CHECK(m_objTool->setProperty("IsoWP","GradientLoose") );
  ...
  CHECK(m_objTool->setProperty("RequireIsoSignal",false));
  ...
  (*configHandle)->initialize().ignore();

  trigDecTool = new Trig::TrigDecisionTool("TrigDecTool2");
  trigDecTool->setProperty("ConfigTool",*configHandle).ignore();
  ...
  // confFiles.push_back("multiLepSearch/mc15a_defaults.NotRecommended.prw.root");
  // lcalcFiles.push_back("multiLepSearch/ilumicalc_histograms_None_267073-271595.root");
  // dynamic_cast<CP::PileupReweightingTool&>(*m_prwTool).setProperty( "ConfigFiles", confFiles);
  // dynamic_cast<CP::PileupReweightingTool&>(*m_prwTool).setProperty( "LumiCalcFiles", lcalcFiles);
  // m_prwTool->initialize();

  /// electron ID SF
  PATCore::ParticleDataType::DataType data_type(PATCore::ParticleDataType::Data);
  if (m_isMC==1) data_type = PATCore::ParticleDataType::Full;
  else if(m_isMC==2) data_type = PATCore::ParticleDataType::Fast;

  m_elecEfficiencySFTool_loose = new AsgElectronEfficiencyCorrectionTool("electron_ID_SF_loose");
  std::vector< std::string > corrFileNameList_id;
  corrFileNameList_id.push_back("ElectronEfficiencyCorrection/efficiencySF.offline.LooseLH.2015.13TeV.rel20p0.v02.root");
  CHECK(m_elecEfficiencySFTool_loose->setProperty("CorrectionFileNameList", corrFileNameList_id));
  CHECK(m_elecEfficiencySFTool_loose->setProperty("ForceDataType", (int)data_type));
  CHECK(m_elecEfficiencySFTool_loose->initialize());

  m_elecEfficiencySFTool_tight = new AsgElectronEfficiencyCorrectionTool("electron_ID_SF_tight");
  std::vector< std::string > corrFileNameList_tight;
  corrFileNameList_tight.push_back("ElectronEfficiencyCorrection/efficiencySF.offline.TightLH.2015.13TeV.rel20p0.v02.root");
  CHECK(m_elecEfficiencySFTool_tight->setProperty("CorrectionFileNameList", corrFileNameList_tight));
  CHECK(m_elecEfficiencySFTool_tight->setProperty("ForceDataType", (int)data_type));
  CHECK(m_elecEfficiencySFTool_tight->initialize());

  m_elecEfficiencySFTool_medium = new AsgElectronEfficiencyCorrectionTool("electron_ID_SF_medium");
  std::vector< std::string > corrFileNameList_medium;
  corrFileNameList_medium.push_back("ElectronEfficiencyCorrection/efficiencySF.offline.MediumLH.2015.13TeV.rel20p0.v02.root");
  CHECK(m_elecEfficiencySFTool_medium->setProperty("CorrectionFileNameList", corrFileNameList_medium));
  CHECK(m_elecEfficiencySFTool_medium->setProperty("ForceDataType", (int)data_type));
  CHECK(m_elecEfficiencySFTool_medium->initialize());

  return EL::StatusCode::SUCCESS;
}


EL::StatusCode ljetSelection :: execute ()
{
  ...
  { 
    ...
    //m_susyEvt->evt.weight = m_prwTool->getCombinedWeight(*eventInfo);
    //m_susyEvt->evt.averageMu = eventInfo->averageInteractionsPerCrossing();
  }
  else
  {
    m_susyEvt->evt.averageMu = m_objTool->GetCorrectedAverageInteractionsPerCrossing();
    //m_susyEvt->evt.averageMu = m_prwTool->getLumiBlockMu(*eventInfo);
  }
...
//Assign trigCode
    ...
    for(unsigned int i=0; i<trigNames.size(); i++){
      if(trigDecTool->isPassed(trigNames[i])) 
      {
...
//test lepton & jet
//    std::cout<<"jet: Pt:"<<jet_Ls[0]->pt()<<",eta:"<<jet_Ls[0]->eta()<<",phi:"<<jet_Ls[0]->phi()<<std::endl;
//    if(nEL == 1) std::cout<<"electron: Pt:"<<sel_Ls[0]->pt()<<",eta:"<<sel_Ls[0]->eta()<<",phi:"<<sel_Ls[0]->phi()<<std::endl;
//    if(nMU == 1) std::cout<<"muon: Pt:"<<sel_Ls[0]->pt()<<",eta:"<<sel_Ls[0]->eta()<<",phi:"<<sel_Ls[0]->phi()<<std::endl;
...
//       m_susyEvt->sig.mT2 =  asymm_mt2_lester_bisect::get_mT2(tl1.M(), tl1.Px(), tl1.Py(), tl2.M(), tl2.Px(), tl2.Py(), metFinal->mpx(), metFinal->mpy(), 1, 1)*iGeV;
  ...
  m_store->record(muons_copy, "ShallowCopiedMuons");
  m_store->record(muons_copyaux, "ShallowCopiedMuonsAux.");
  m_store->record(electrons_copy, "ShallowCopiedElectrons");
  m_store->record(electrons_copyaux, "ShallowCopiedElectronsAux.");
  m_store->record(jets_copy, "ShallowCopiedJets");
  m_store->record(jets_copyaux, "ShallowCopiedJetsAux.");
...
EL::StatusCode ljetSelection :: postExecute ()
{
...
EL::StatusCode ljetSelection :: finalize ()
{
...
//   for(int i=0; i<nTrig; i++) std::cout << i+1<<" "<<trigNames[i]<<":   " <<trigCount[i] <<std::endl;
  if(m_elecEfficiencySFTool_loose) delete m_elecEfficiencySFTool_loose;
  if(m_elecEfficiencySFTool_tight) delete m_elecEfficiencySFTool_tight;
  if(m_elecEfficiencySFTool_medium) delete m_elecEfficiencySFTool_medium;
...
EL::StatusCode ljetSelection :: histFinalize ()
{
...
void ljetSelection :: fillLepton(xAOD::IParticle* p, L_PAR& l, unsigned int /*index*/)
{
    l.pt = p->pt()*iGeV;
    l.eta = p->eta();
    l.phi = p->phi();
      ...
      if(m_isMC)
       {
         ...
         if(trk){
          ...
          l.truthType = acc_truthType.isAvailable(*trk)? acc_truthType(*trk):0;
          l.truthOrig = acc_truthOrig.isAvailable(*trk)? acc_truthOrig(*trk):0;
         }
...
//         if(el->isGoodOQ(el->OQ())) l.lFlag |= EL_OQ;
        ...
        if(m_isMC)
        {
          //l.SF_Loose = m_objTool->GetSignalElecSF(*el, true, true,false, false);
          //l.SF_Tight = m_objTool->GetSignalElecSF(*el,true, true, true, true);
          
          //l.truthType = acc_truthType(*el);
          l.truthType = acc_truthType.isAvailable(*el)? acc_truthType(*el):0;
          l.truthOrig = acc_truthOrig.isAvailable(*el)? acc_truthOrig(*el):0;
...
//           l.truthProb = trk?acc_truthProb(*trk):-1;
        ...
        double id_sf(-999.);
        l.wt1 = (m_elecEfficiencySFTool_loose->getEfficiencyScaleFactor(*el, id_sf) == CP::CorrectionCode::Ok)?id_sf:-999.;
        l.wt2 = (m_elecEfficiencySFTool_tight->getEfficiencyScaleFactor(*el, id_sf) == CP::CorrectionCode::Ok)?id_sf:-999.;
        l.wt3 = (m_elecEfficiencySFTool_medium->getEfficiencyScaleFactor(*el, id_sf) == CP::CorrectionCode::Ok)?id_sf:-999.;
        if(m_isMC)
        {
          l.SF_Loose = m_objTool->GetSignalElecSF(*el,true,true,true,false);
          l.SF_Tight = l.SF_Loose / l.wt1 * l.wt2;
          l.SF_Medium = l.SF_Loose / l.wt1 * l.wt3;
        }
      }
     }
...
int ljetSelection::addTruthPar(const xAOD::TruthParticle* p, TRUTHS& v, int pLevel){
