...
TString defaultNTupleList="../common/inFileList-MCdR.txt";
TString defaultMisIdfile="misIDdecorations.root";
...
class Histos
{
  ...
  public:
   ...
   void Fill(const double value, const bool SSevent){
      if(SSevent){
         SS->Fill(value, evt2lTree->evt_weight*evt2lTree->evt_pwt);
      } else{
         ExpSS->Fill(value, (evt2lTree->evt_weight*evt2lTree->evt_pwt)*(misIDTree->chargeFlipWeight));
         OS->Fill(value, evt2lTree->evt_weight*evt2lTree->evt_pwt);
      }      
   }
   ...
   void Draw(){
      ...
      RatioY->SetRangeUser(0,2);
      RatioY->SetNdivisions(208);
...
// ========= FUNCTIONS =========== //
void extractSSprediction(const TString outputDir, const TString ntupleList, const TString misIDfile, const TString opt);
bool initialize(const TString outputDir, const TString ntupleList, const TString misIDfile, const TString opt);
...
void extractSSprediction(const TString outputDir=defaultOut, 
							const TString ntupleList=defaultNTupleList, 
							const TString misIDfile=defaultMisIdfile,
                     const TString opt="")
{
   
   if(!initialize(outputDir, ntupleList, misIDfile, opt)){
      std::cout << "Exited from initialize()" << endl;
      return;
   }

   // Initialize histograms here ----------------------------------------
   Histos hMass("hMass", "invariant mass", "m_{ll}", "Events/GeV", 100, 20, 220);
   Histos hLeadingPt("hLeadingPt", "leading p_{T}", "p_{T}", "Events/GeV", 100, 20, 200);
   Histos hSubleadingPt("hSubleadingPt", "subleading p_{T}", "p_{T}", "Events/GeV", 100, 20, 200);
   Histos hLeadingEta("hLeadingEta", "leading |#eta|", "Leading |#eta|", "", 50, 0, 2.47);
   Histos hSubleadingEta("hSubleadingEta", "subleading |#eta|", "Subleading |#eta|", "", 50, 0, 2.47);
   Histos hLeadingPhi("hLeadingPhi", "leading #phi", "Leading #phi", "", 50, -3.15, 3.15);
   Histos hSubleadingPhi("hSubleadingPhi", "subleading #phi", "Subleading #phi", "", 50, -3.15, 3.15);

   long long nEntries = evt2lTree->fChain->GetEntries();
   for(long long i=0; i<nEntries; i++){
      loadbar(i+1, nEntries);
      evt2lTree->GetEntry(i);
      misIDTree->GetEntry(i);
      ...
      //    if(evt2lTree->jets_jFlag[i] & 2) nJets++;
      //    if(evt2lTree->jets_jFlag[i] & 32) nBJets++;
      // }
      // if(nJets<2) continue;
      ...
      if(applyPtCorrection && !SSevent){
         if(misIDTree->e1rate > misIDTree->e2rate){
            pt1 += misIDTree->e1dPt;
         } else {
            pt2 += misIDTree->e2dPt;
         }
      ...
      hMass.Fill(mll, SSevent);
      hLeadingPt.Fill(pt1, SSevent);
      hSubleadingPt.Fill(pt2, SSevent);
      hLeadingEta.Fill(fabs(evt2lTree->leps_eta[0]), SSevent);
      hSubleadingEta.Fill(fabs(evt2lTree->leps_eta[1]), SSevent);
      hLeadingPhi.Fill(evt2lTree->leps_phi[0], SSevent);
      hSubleadingPhi.Fill(evt2lTree->leps_phi[1], SSevent);
   }
...
bool initialize(const TString outputDir, const TString ntupleList, const TString misIDfile, const TString opt)
{
   ...
   // Get misID tree 
   ///////////////////////////////////
   TFile* fMisID = TFile::Open(misIDfile);
   if(!fMisID){
      std::cout << "Could not open " << misIDfile << endl;
      return false;
   }
   TTree* tMisID = (TTree*) fMisID->Get("misIDvars");
   if(!tMisID){
      std::cout << "Could not get TTree misIDvars " << misIDfile << endl;
      return false;
   }
   misIDTree = new misIDvars(tMisID);
   if(!misIDTree){
   	std::cout << "Could not create skeleton class for misIDvars" << endl;
   	return false;
   }
   if(misIDTree->fChain->GetEntries()!=evt2lTree->fChain->GetEntries()){
   	std::cout << "Different number of events in misIDTree and evt2lTree" << endl;
   	std::cout << "Cannot do analysis" << endl;
   	return false;
   }
   ...
   if(!fOut){
      std::cout << "Could not create output file " << outputDir << "/SSpredictions.root" << endl;
      return false;
   }
   ...
   // Everything okay!
   std::cout << "initialize() complete" << endl;
   return true;
}
