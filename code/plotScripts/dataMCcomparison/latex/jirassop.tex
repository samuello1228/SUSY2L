...
\begin{itemize}
\item Optimize the SR for 3 signal points.
\item Start from run 1 SR.
\item ?????
\end{itemize}
\end{frame}
...
\begin{itemize}
\item At least 10 raw events for VV and ttV.
\item If the weighted yield for a type of BG is negative, set the weighted yield to 0.
\item nSig$>$1 and nSig/nSigError $>$ 2.
\item For \pt 1, \pt 2, ptll, $m_{T2}$, $E_{\text{T}}^{\text{miss,rel}}$, $m_{\text{eff}}$ and $m_{\text{T}}^{\text{max}}$, there is no upper cut.
