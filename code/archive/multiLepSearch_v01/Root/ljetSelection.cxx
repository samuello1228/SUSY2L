...
//   Info("initialize()", "DataSource = %d", m_isMC);
  ...
  ST::SettingDataSource ds = static_cast<ST::SettingDataSource>(m_isMC); 
...
EL::StatusCode ljetSelection :: execute ()
{
  ...
  for(auto jet: *jets_copy){
    if(!m_objTool->IsSignalJet(*jet, JetPtCut, JetEtaCut, JetJvtCut) || !dec_passOR(*jet)) continue;
    nJET++;
...
//       l.ID += mu->quality();
      ...
      if(m_objTool->IsBadMuon(*mu)) l.lFlag |= IS_BAD;
      if(m_objTool->IsCosmicMuon(*mu)) l.lFlag |= MU_COSMIC;
