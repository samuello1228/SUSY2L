...
//   m_objTool->msg().setLevel( MSG::VERBOSE );
  ...
  CHECK(m_prwTool->setProperty( "ConfigFiles", PRW_confFiles));
  CHECK(m_prwTool->setProperty( "LumiCalcFiles", PRW_lcalcFiles));
  CHECK(m_prwTool->setProperty( "DataScaleFactor", 1.0/1.16));
  CHECK(m_prwTool->initialize());
...
void evtSelection :: fillLepton(xAOD::IParticle* p, L_PAR& l, unsigned int index)
{
    l.pt = p->pt()*iGeV;
    l.eta = p->eta();
    l.phi = p->phi();
